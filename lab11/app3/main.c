  ...
  while (go_condition)
  {
    ...
    switch (option)
    {
      case 1:
      {
        ...
        printf("\t - enter the person's gender (f/m): ");
        buffer_gender = getchar();
        getchar(); // < clears \n from after gender input;
        ...
        return_value = addPerson(buffer_name, buffer_gender, buffer_wage);
        if (!return_value)
        {
          free(buffer_name);
          MALLOC_COUNTER -= 2;
      ...
      case 5:
      {
        ...
        else
        {
          ...
          return_value = addPerson(new_name, new_gender, new_wage);
          if (!return_value)
          {
            free(old_name);
            free(new_name);
