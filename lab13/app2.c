...
#define FIND_SHORTEST_LENGTH 0
#define PRINT_SHORTEST_SOLUTIONS 1
...
int shortest_solution_length = INT_MAX;
...
void printSolution(Point * solution, const int * length)
{
  int i;
  ...
  for (i = 0; i < *length; i++)
  {
    labyrinth [solution [i].x][solution [i].y] = 'a' + i;
  }
  ...
  for (i = 0; i < *length; i++)
  {
    labyrinth [solution [i].x][solution [i].y] = '.';
  }
}

// Backtracking function - computes the path out of the labyrinth;
void backtrack(Point current_position, int level, int calling_mode)
{
  ...
  */
  ...
  if (level > shortest_solution_length)
  {
    return;
  }
            ...
            labyrinth [current_position.x][current_position.y] == ROAD)
  {
    ...
    if (level < shortest_solution_length && calling_mode == FIND_SHORTEST_LENGTH)
    {
      // Setting the length of the shortest distance;
      shortest_solution_length = level;
    }
    else if (level == shortest_solution_length && calling_mode == PRINT_SHORTEST_SOLUTIONS)
    {
      // Printing a shortest distance solution;
      ...
      printf("Solution #%d;\n", solution_counter++ + 1);
      printSolution(visited_points, &shortest_solution_length);
    }
  }
  ...
  else
  {
    ...
    for (i = 0; i < DIRNUMBER; i++)
    {
      ...
      if (checkPosition(&next_position, &level))
      {
        ...
        // Resizing the array of visited points to have space for the new position;
        ...
        backtrack(next_position, level + 1, calling_mode);
      }
    }
  }
}
...
int main()
{
  ...
  backtrack(starting_point, 1, FIND_SHORTEST_LENGTH);
  printf("The length of the shortest road is %d;\n\n", shortest_solution_length);
  backtrack(starting_point, 1, PRINT_SHORTEST_SOLUTIONS);
